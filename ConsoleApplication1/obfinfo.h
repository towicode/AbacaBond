#pragma once


char * playerHolderObf = "client;";
char * playerClassObf = "t;";

char* ClientObf = "Lclient;";
char* CanvasHolderObf = "Lav;";
char* PlayerHolderObf = "Lav;";
char* CharacterObf = "Lt;";
char* ChracaterArrayObf = "[Lt;";
char* CanvasObf = "Ljava/awt/Canvas;";
char* MouseListenerObf = "Ljava/awt/event/MouseListener;";
char* MouseMotionListenerObf = "Ljava/awt/event/MouseMotionListener;";
char* MouseEventObf = "Ljava/awt/event/MouseEvent;";


char * localPlayerHolderObf = "Lf;";
char * localPlayerAcessor = "hb";


char * canvasHolderObf = "Lbb;";
char * canvasAcessor = "qz";



//	used to access localPlayer
char* localCharField = "hl";
char* localCanvasField = "qc";

//	Player fields
char * player_name = "f";
char * playersAccessorObf = "gs";
